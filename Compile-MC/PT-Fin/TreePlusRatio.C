...
char cut[20];
strcpy(cut, "0.01<mc_pT&&mc_pT<0.1");   // First cut applied on variable of focus, change for different variable

float start = 0.1;      // Larger value of first cut made on target variable
float end = 1.5;        // Number one increment greater than the larger value of the final cut (getting around the for-loop being not inclusive)
...
float N2;
	...
	outputfile = fopen("pT-ACT-Tree.txt" , "wb");  // Text file to print to

	for (float j = start; j < end; j += increment) {  // Loop to run through the target variable cuts
		
        	TCut VARcut = (cut);

        	TH1F*h1 = new TH1F("h1","h1",500, 0, 1.5);  // Histogram for mean of GENERATED pT
        	outTree->Draw("mc_pT>>h1", VARcut);
		mean1 = h1->GetMean();
		std1 = h1->GetStdDev();
		N1 = h1->GetEntries();

        	TH1F*h2 = new TH1F("h2","h2",500, 0, 1.5);  // Histogram for mean of RECONSTRUCTED pT
        	outTree->Draw("pT>>h2", VARcut);
		mean2 = h2->GetMean();
		std2 = h2->GetStdDev();
		N2 = h2->GetEntries();
		...
		sprintf(cut, "%f<mc_pT&&mc_pT<%f", j, j + increment);
		printf("%s \n", cut);
...
FILE * fp1 = fopen("pT-ACT-Tree.txt", "r");
