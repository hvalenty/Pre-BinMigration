...
void OneDimBoth(){
	...
	Float_t e_p, mc_e_p, e_theta, mc_e_theta, p1_p, mc_p1_p, p1_theta, mc_p1_theta, Q2, mc_Q2, W, mc_W, Mx, mc_Mx, x, mc_x, y, mc_y, z, mc_z, xF, mc_xF, pT, mc_pT, zeta, mc_zeta, eta, mc_eta, trento_phi, mc_trento_phi, matching_e_pid, matching_p1_pid, mc_p1_parent, dummy, mc_dummy, delta_p, delta_pT, delta_Q2;
...
// Read in file
	...
	outTree->Branch("delta_pT",&delta_pT,"delta_pT/F");
...
for (int k=0; k<5000000; k++) {
	...
	delta_pT = pT - mc_pT; //reconstructed - generated
...
gStyle->SetPalette(1);
c00->Divide(1,1);
TH1D*h1 = new TH1D("h1","", 500, 0.01, 2);
TH1D*h2 = new TH1D("h2","", 500, 0.01, 2);
TH1D*h3 = new TH1D("h3","", 400, -0.1, 0.1);

c00->cd(1);
h1->GetXaxis()->SetTitle("mc_p_{T}");  // The "#" format will print the symbol
...
h1->SetLineColor(4);
h1->SetName("h1");
outTree->Draw("mc_pT>>h1");
h1->Draw();

h2->GetXaxis()->SetTitle("p_{T}");  // The "#" format will print the symbol
...
TCanvas*c01 = new TCanvas("c01", "", 1000,700);
c01->cd(1);
h3->GetXaxis()->SetTitle("#delta p_{T}");  // The "#" format will print the symbol
...
h3->SetStats(1);
h3->SetLineColor(1);
outTree->Draw("delta_pT>>h3");


c00->Print("pT-ACT.png");
c01->Print("DeltapT-ACT.png");
}
