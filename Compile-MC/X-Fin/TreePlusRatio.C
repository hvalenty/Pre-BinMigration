...
char cut[20];
strcpy(cut, "0.1<mc_x&&mc_x<0.16");   // First cut applied on variable of focus, change for different variable

float start = 0.16;      // Larger value of first cut made on target variable
float end = 0.76;        // Number one increment greater than the larger value of the final cut (getting around the for-loop being not inclusive)
float increment = 0.06;  // Size of cut
...
float N2;
	...
	outputfile = fopen("X-ACT-Means.txt" , "wb");  // Text file to print to

	for (float j = start; j < end; j += increment) {  // Loop to run through the target variable cuts
		
        	TCut VARcut = (cut);

        	TH1F*h1 = new TH1F("h1","h1",500, 0, 0.7);  // Histogram for mean of GENERATED pT
        	outTree->Draw("mc_x>>h1", VARcut);
		mean1 = h1->GetMean();
		std1 = h1->GetStdDev();
		N1 = h1->GetEntries();

        	TH1F*h2 = new TH1F("h2","h2",500, 0,0.7);  // Histogram for mean of RECONSTRUCTED pT
        	outTree->Draw("x>>h2", VARcut);
		mean2 = h2->GetMean();
		std2 = h2->GetStdDev();
		N2 = h2->GetEntries();
		...
		sprintf(cut, "%f<mc_x&&mc_x<%f", j, j + increment);
		printf("%s \n", cut);
...
FILE * fp1 = fopen("X-ACT-Tree.txt", "r");
...
h3->Draw("AP");
h3->GetXaxis()->SetRangeUser(0.1,0.7);
h3->GetYaxis()->SetRangeUser(0.8, 1.2);
gPad->Modified();
gPad->Update();
//c01->Print("X-ACT-Tree.png");
